...
def evaluate_models(
    ...
    evaluation = {
        ...
        "limitations": [
            f"В текущем исследовательском срезе доступны {len({window['patient_id'] for window in windows})} пациентов OhioT1DM, поэтому межпациентская переносимость остаётся ограниченной.",
...
def _legacy_make_about_payload_unused(encoder: FeatureEncoder, settings: Settings) -> dict[str, Any]:
    return {
        ...
        "why_retrieval_justified": [
            "При малом числе пациентов и ограниченном числе окон переусложнение модели повышает риск нестабильной оценки и переобучения.",
        ...
        "limitations": [
            ...
            "Текущий 12-пациентный срез не поддерживает сильные внешние обобщения.",
        ],
    }
